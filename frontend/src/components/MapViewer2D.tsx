...
import { MapPin, Navigation, Compass, Crosshair, AlertCircle, Info, Activity, Satellite } from 'lucide-react';
  ...
  const renderMap = useCallback(() => {
    ...
    for (let y = 0; y < grid_size; y++) {
      for (let x = 0; x < grid_size; x++) {
        ...
        if (elev < 15.0) {
          // Krishna River Channel: Dark Sandy Slate
          ctx.fillStyle = '#0f172a';
        } else if (elev < 26.0) {
          // Vijayawada / Tadepalli Plain: Urban Slate
          const g = Math.floor(28 + normElev * 40);
          ctx.fillStyle = `rgb(18, ${g}, 32)`;
        } else {
          // Indrakeeladri / Gunadala Hills: Rocky Tan/Brown
          const r = Math.floor(45 + normElev * 60);
          const g = Math.floor(35 + normElev * 40);
    ...
    // 3. Observed Satellite Flood Overlay (ISRO / Sentinel-1)
    ...
    if (layerVisibility.roads) {
      ...
      roads.forEach((road) => {
        ...
        ctx.lineWidth = road.critical_evacuation_route ? 3.0 : 1.8;
        ctx.beginPath();
        ctx.moveTo(sx, sy);
        ctx.lineTo(ex, ey);
        ctx.stroke();
    ...
    // 7. Critical Facilities & Landmarks
    ...
    <div ref={containerRef} className="relative w-full h-full bg-[#070d18] overflow-hidden select-none">
      ...
      <div className="absolute top-4 left-4 z-10 flex flex-col space-y-2">
        <div className="glass-panel px-3 py-2 rounded-xl text-xs flex items-center space-x-3 text-slate-300 shadow-xl border border-slate-800">
          <div className="flex items-center space-x-1.5">
            ...
            <span className="font-semibold text-slate-100">{gisData.river} — {gisData.location}</span>
          </div>
          <span className="text-slate-600">•</span>
          <span>Zoom: <strong className="font-mono text-cyan-400">{zoom.toFixed(1)}x</strong></span>
          <span className="text-slate-600">•</span>
          <span className="text-slate-400">Click anywhere to probe point hydrograph</span>
        </div>
      </div>
      ...
      <div className="absolute bottom-6 left-4 z-10 glass-panel p-3 rounded-xl shadow-2xl text-xs space-y-2 w-64 border border-slate-800">
        <div className="flex justify-between items-center text-[11px] font-semibold text-slate-300">
          <span>Krishna River Inundation Depth</span>
        ...
        <div className="absolute top-4 right-4 z-20 glass-panel p-4 rounded-xl shadow-2xl border border-cyan-500/30 w-72 text-xs space-y-3 animate-in fade-in slide-in-from-top-2">
          <div className="flex items-center justify-between border-b border-slate-800 pb-2">
            <span className="font-bold text-cyan-400 flex items-center space-x-1.5">
              ...
              <span>Vijayawada Basin Point Probe</span>
            </span>
            <button
          ...
          <div className="grid grid-cols-2 gap-2 text-slate-300">
            ...
            <div className="bg-slate-900/80 p-2 rounded-lg border border-slate-800">
              <span className="text-[10px] text-slate-400 block">Peak Inundation</span>
